...
class Annotator():
    ...
    def draw_landmarks(self):
        for p in self.lm:
            ...
            cv2.circle(self.im, point, self.ps, Color.green, -1)
    ...
    def draw_info(self, fontColor=Color.red):
        x, y, z = self.angles
        px, py, dy = int(5 * self.fs), int(25 * self.fs), int(30 * self.fs)
        font = cv2.FONT_HERSHEY_DUPLEX
        fs = self.fs
        cv2.putText(self.im, "Pitch: %+06.2f" % x, (px, py), font, fontScale=fs, color=fontColor)
        cv2.putText(self.im, "Yaw: %+06.2f" % y, (px, py + dy), font, fontScale=fs, color=fontColor)
        cv2.putText(self.im, "Roll: %+06.2f" % z, (px, py + 2 * dy), font, fontScale=fs, color=fontColor)
